...
export default ({
  ...
  const getUrlSummary = (event) => {
    ...
    fetch(process.env.REACT_APP_API + 'youtubeUrl/', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      ...
      .then((result) => {
        ...
        fetch(process.env.REACT_APP_API + 'youtubeUrl/')
          .then((res) => res.json())
          .then((data) => {
            fetch(process.env.REACT_APP_API + 'youtubeUrl/' + data[data.length-1].urlid)
              .then((res) => res.json())
              .then((data) => {seturlSummary(data.urlSummary);seturlTranscript(data.urlTranscript);setshowLoading(false)})
              .catch((error) => console.log(error));
          })
          .catch((error) => console.log(error));
      },
        ...
        >
          <Box sx={style} className="box">
            ...
            <div>
              ...
              </Typography> : input === 'YoutubeUrl' ? <Typography variant="h6" component="h4">
                You have selected {input} as Input format
            ...
            <br></br>
             ...
             <div className="parent">
                    ...
                    <div className="child2">
                    <h5>SUMMARY</h5>
                      <textarea className="intext" value={urlSummary} placeholder="Summary....."></textarea>
                    </div>
            </div>
            <div className="parent">
                    <div className="child1">
                    <h5>TRANSCRIPT</h5>
                      <textarea className="intext"  value={urlTranscipt} placeholder="Transcript of the video..."></textarea>
                    </div>
            </div>
            <br></br>
